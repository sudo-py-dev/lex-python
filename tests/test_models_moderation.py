...
@pytest.mark.asyncio
async def test_mediafilter_creation(db_session):
    mf = MediaFilterFactory.build(chatId=-600, blockPhoto=True)
    db_session.add(mf)
    await db_session.flush()
    
    result = await db_session.execute(select(MediaFilter).where(MediaFilter.chatId == -600))
    fetched = result.scalar_one()
    assert fetched.blockPhoto is True
